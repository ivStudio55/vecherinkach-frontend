...
code = code.replace(
  ...
  '<div className="max-w-6xl mx-auto px-4 sm:px-6 min-h-[100dvh] flex flex-col justify-center py-4 lg:py-6 gap-4">'
);

// We want to completely replace the third panel (packCards + partyGames section)
const startMarker = '<div className={panelEnterClass(panelStage >= 3)} style={panelEnterStyle(panelStage >= 3, 280)}>';
const endMarker = '</div>\n          </div>\n        )}'; // Up to the end of the `hasStarted` branch
const idxStart = code.indexOf(startMarker);
const idxEnd = code.indexOf(endMarker, idxStart);
