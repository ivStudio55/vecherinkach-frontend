...
const ROUND3_DISABLED = false; // включаем механику Раунда 3 (было временно отключено)
...
export default function HostRoomPage() {
  ...
  const [round3Votes, setRound3Votes] = useState<Round3VoteRow[]>([]);
  ...
  const round3VotesRef = useRef<Round3VoteRow[]>([]);
  ...
  useEffect(() => {
    round3VotesRef.current = round3Votes;
  }, [round3Votes]);
  ...
  const stopAllRound3Audio = useCallback(() => {
    // Останавливаем вообще всё, что может играть в 3 раунде
    [
      round3QuestionAudioRef,
      round3QuestionBgAudioRef, // round3BedAudioRef
      round3TimerAudioRef,
      round3VoteAudioRef,
      round3VoteTimerAudioRef,
      round3CommentAudioRef,
      round3CommentBgAudioRef,
      round3RulesAudioRef // на всякий случай
    ].forEach(ref => {
      if (ref.current) {
        ref.current.pause();
    ...
    // Также сбрасываем обработчики
    if (round3QuestionBgAudioRef.current) round3QuestionBgAudioRef.current.onended = null;
    if (round3QuestionAudioRef.current) round3QuestionAudioRef.current.onended = null;
  }, []);
  ...
  const playRound3VoteAudio = useCallback(() => {
    ...
    setTimeout(() => {
      // По окончании 15 секундного таймера все звуки отключаются
      stopRound3VoteAudio();
  ...
  const startRound3Timer = useCallback(
    (duration: number, onComplete?: () => void) => {
      ...
      if (round3QuestionBgAudioRef.current) {
          round3QuestionBgAudioRef.current.pause();
      }
  ...
  const loadRound3Votes = useCallback(
    async (questionIndex: number | null) => {
      ...
      const { data, error } = await supabase
        .from('round3_votes')
        .select('player_id, answer_id, question_index')
        .eq('room_id', roomId)
        .eq('question_index', questionIndex);

      if (error) {
        console.error('[Round3] Не удалось загрузить голоса', error);
        return;
      }
      setRound3Votes((data as Round3VoteRow[]) || []);
    },
    [roomId]
  );

  const loadRound3VotesRef = useRef(loadRound3Votes);
  useEffect(() => {
    loadRound3VotesRef.current = loadRound3Votes;
  }, [loadRound3Votes]);

  // Автотаймер голосования на основе round3_vote_started_at (устойчиво к обновлению страницы ведущего)
  useEffect(() => {
    if (roomStatus !== 'round3-voting') {
      return;
    }
    if (!round3VoteStartedAt) {
      return;
    }
    if (round3TimerRef.current) {
      return;
    ...
    const tick = () => {
      const startMs = new Date(round3VoteStartedAt).getTime();
    ...
    tick();
    round3TimerRef.current = setInterval(tick, 250);
    return () => {
      if (round3TimerRef.current) {
        clearInterval(round3TimerRef.current);
        round3TimerRef.current = null;
  ...
  }, [clearRound3Timer, roomStatus, round3VoteStartedAt, startRound3Reveal, stopRound3VoteAudio, timeOffsetMs]);

  // При входе в reveal подгружаем голоса, чтобы посчитать очки тура
  useEffect(() => {
    if (roomStatus !== 'round3-reveal') {
      return;
    }
    void loadRound3VotesRef.current?.(round3CurrentIndexRef.current);
  }, [roomStatus]);
  ...
  const startRound3Voting = useCallback(async () => {
    ...
    round3CurrentIndex,
    round3Phase, // Keep round3Phase in deps even if not used in logic, to be safe or remove if linter complains
    startRound3Reveal,
    stopAllRound3Audio,
    stopRound3VoteAudio,
  ]);
  ...
  const handleRound3Cancel = useCallback(async () => {
    clearRound3Timer();
    stopAllRound3Audio();
    ...
    setShowResults(true);
  }, [clearRound3Timer, roomId, setRound3CurrentIndex, setRoomStatus, stopAllRound3Audio]);
  ...
  const handleRound3Button = useCallback(() => {
    if (players.length < ROUND3_MIN_PLAYERS) {
      setRound3Notice('Для Раунда 3 нужно минимум 3 игрока.');
            ...
            {showResults ? (
              ...
              <div className="rounded-3xl border-[4px] border-[#b4007f] bg-white shadow-xl p-6 space-y-5">
                ...
                <div className="rounded-3xl border-[4px] border-[#1f6ac6] bg-white shadow-xl p-6 space-y-6">
                  ...
                  <div className="rounded-3xl border-[3px] border-[#142a45]/15 bg-white p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="retro-heading text-[11px] tracking-[0.4em] text-[#142a45]/70">Итоги тура · Очки (Раунд 3)</p>
                      <span className="text-xs font-semibold text-[#1f6ac6]">{round3FactScoreboard.length ? 'готово' : 'ждём данные'}</span>
                    </div>
                    {round3FactScoreboard.length === 0 ? (
                      <p className="text-sm text-[#142a45]/70">Очки появятся после завершения голосования.</p>
                    ) : (
                      <div className="space-y-2">
                    ...
                    <p className="text-xs text-[#142a45]/60">
                      Правила: точное слово +200, каждый голос +50, пропуск голосования -50.
                    </p>
                  </div>
                  ...
                  <div className="rounded-3xl border-[4px] border-[#142a45] bg-white shadow-xl p-6 space-y-5">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          void handleRound3Cancel();
                        }}
                        className="flex-1 py-4 rounded-2xl font-black text-xl tracking-[0.2em] bg-white text-[#b4007f] border-[3px] border-[#142a45]"
                      >
                        Отмена (сброс)
                      </button>
                    </div>
