  ...
  useEffect(() => {
    ...
    const poll = async () => {
      const { data } = await supabase
        .from('rooms')
        .select(
          'id, pack_id, current_question_index, is_active, status, question_started_at, all_players_answered, selected_question_ids, round2_item_index, round2_showing_fact, round2_phase, round5_question_index'
        )
        .eq('id', roomId)
        .single();
      if (mounted && data) {
