...
export async function GET(request: Request) {
  ...
  const roomQuery = supabase.from('rooms').select('id, code, status, is_active, created_at, pack_id, state_version, transitioning_to_next, current_question_index, question_started_at, all_players_answered, selected_question_ids, round2_item_index, round2_showing_fact, round2_phase').eq('id', roomId).maybeSingle();
