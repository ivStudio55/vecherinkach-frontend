...
export default function JokesterPacksAdminPage() {
  const router = useRouter();
  const [packs, setPacks] = useState<JokesterPackRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [toast, setToast] = useState<string | null>(null);
  const [editPack, setEditPack] = useState<Partial<JokesterPackRow> | null>(null);
  ...
  const getPackUrl = (packId: string) => `https://vecherinkach.ru/jokester?pack=${encodeURIComponent(packId)}`;
  ...
  const loadPacks = useCallback(async () => {
    const data = await apiFetch('/api/panel/jokester-packs');
  ...
  const togglePublic = async (pack: JokesterPackRow) => {
    await apiFetch('/api/panel/jokester-packs', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const deactivatePack = async (id: string) => {
    ...
    await apiFetch(`/api/panel/jokester-packs?id=${encodeURIComponent(id)}`, { method: 'DELETE' });
    showToast('✅ Пакет деактивирован');
    loadPacks();
  };

  const validatePack = async (pack: JokesterPackRow) => {
    setValidating(pack.id);
    let ok = false;
    try {
      const res = await fetch(pack.json_url, { method: 'HEAD', mode: 'cors' });
      ok = res.ok;
    } catch {
      ok = false;
    }
    setValidationResults(prev => ({ ...prev, [pack.id]: ok }));
    setValidating(null);
  };
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <header className="bg-gray-900/80 backdrop-blur border-b border-gray-800 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold">🤡 Пакеты вопросов Пошутикач</h1>
          </div>
          <button
            onClick={() => setEditPack({
              id: '',
      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 text-sm text-gray-300 space-y-2">
          <p className="font-bold text-white">📋 Как добавить новый пакет вопросов для Пошутикач:</p>
          <ol className="list-decimal list-inside space-y-1">
            <li>Подготовьте JSON файл <code className="bg-gray-700 px-1 rounded">jokester_questions.json</code> с массивом категорий</li>
            <li>Загрузите его в <code className="bg-gray-700 px-1 rounded">{JSON_CDN_BASE}/jokester_questions_pack/ID_ПАКЕТА/jokester_questions.json</code></li>
            <li>Нажмите «+ Новый пакет», заполните форму</li>
            <li>Проверьте пакет кнопкой «Проверить» — файл должен быть зелёным</li>
            <li>Установите «Публичный» для отображения в списке выбора</li>
          </ol>
          <p className="text-gray-400 mt-2">Формат JSON: <code className="bg-gray-700 px-1 rounded">{`{ "categories": [{ "id": "...", "name": "...", "emoji": "...", "questions": ["..."] }] }`}</code></p>
        </div>
          ...
          <div key={pack.id} className={`border rounded-xl p-4 space-y-3 ${pack.is_active ? 'border-gray-700 bg-gray-900/50' : 'border-red-900/50 bg-red-950/20 opacity-60'}`}>
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 font-mono break-all">URL: {pack.json_url}</p>
              </div>
            ...
            {/* Validation result */}
            {validationResults[pack.id] !== undefined && (
              <div className="flex flex-wrap gap-2">
                <span className={`px-2 py-1 rounded text-xs font-mono ${validationResults[pack.id] ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                  {validationResults[pack.id] ? '✅' : '❌'} jokester_questions.json
                </span>
              </div>
            )}
          </div>
      ...
      {editPack && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setEditPack(null)}>
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-lg space-y-4" onClick={e => e.stopPropagation()}>
            <h2 className="text-lg font-bold">{packs.some(p => p.id === editPack.id) ? '✏️ Редактировать пакет' : '🤡 Новый пакет Пошутикач'}</h2>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="например: birthday_2026"
              ...
              <div>
                <label className="text-xs text-gray-400 block mb-1">JSON URL (полный путь до jokester_questions.json)</label>
                <input
                  value={editPack.json_url || ''}
