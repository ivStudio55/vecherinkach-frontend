...
code = code.replace(
  ...
  '<div className="max-w-7xl mx-auto px-4 sm:px-6 min-h-[100dvh] flex flex-col justify-center py-4 lg:py-6 gap-4">'
);
...
const idxEnd = code.indexOf(endMarker);
...
if (idxStart !== -1 && idxEnd !== -1) {
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 text-[10px] sm:text-xs font-semibold text-[#1f6ac6] w-full mt-auto">
                        <span className="truncate flex-1">
                          {game.id === 'survivach' ? 'настроить и играть' : 
  ...
  code = code.substring(0, idxStart) + newPanel + closingHTML + code.substring(idxEnd + endMarker.length);
}

fs.writeFileSync('app/page.tsx', code, 'utf8');
console.log('done safe dom replacement');
